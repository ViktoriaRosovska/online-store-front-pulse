...
const CustomForm = ({registr, seeMail}) => {
  ...
  const validationsShema = Yup.object().shape({
    name: Yup.string().typeError('Повинно бути строкою').matches(/([а-яА-яa-zA-z])/,
      {message: 'Повинні бути тільки букви'}
      ).required('обовʼязкове поле'),
    surname: Yup.string().typeError('Повинно бути строкою').matches(/([а-яА-яa-zA-z])/,
      ...
      <Formik initialValues={{
        ...
        {({values, errors,
          ...
          <div className={'input_container form'}>
            ...
            {touched.surname && errors.surname && <p style={{color: 'red'}}>{errors.surname}</p>}
            ...
            <p>
              ...
              <label htmlFor={'password'}>
                {<FaEye className={'eyes1'} style={{cursor: 'pointer'}} onClick={handleClickShowPassword}/>}
                {'Повторити пароль'}
              </label>
            ...
            {touched.passwordCheck && errors.passwordCheck && <p style={{color: 'red'}}>{errors.passwordCheck}</p>}
